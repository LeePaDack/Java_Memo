...
public class GoodsShop {
	...
	public static void main(String[] args) {
		...
		// 물품 목록 보기 Map.Entry
		// Map<String, Integer> = Map.Entry<안에 들어있는 타입이 Map 과 동일해야함>
		//      아이템을 하나씩 키 갚으로 꺼내보기     안에 들어있는 아이템 모두보기
		for(Map.Entry<String, Integer> e : 아이템.entrySet()) {
			String 제품 = e.getKey(); // 제품 명만 가져옴
		...
		String 제품명 = "오미자볼펜";
		// 오미자볼펜이 있는지 확인하고 얼마인지 얘기함
		// 내가 가진 아이템에서 손님이 찾는 제품명이 판매하기위해 존재하는지 확인
		if(아이템.containsKey(제품명)) { // 아이템이 판매로 존재한다면
			int 제품가격 = 아이템.get(제품명); // 오미자볼펜을 검색했을 때 있으면 정보가져오기
