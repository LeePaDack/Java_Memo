...
public class PropTest {
	...
	public static void main(String[] args) {
		...
		// po.fileSave(); 는 po 밑에서 추가로 가져오거나 작성할 때 사용하는것
	...
	public static void fileSave(Properties p) {
		...
		try {
			...
			fos.close();
			System.out.println("설정이 drama.xml 에 저장되었습니다.");
		} catch (Exception e) {
	...
	public static void fileOpen(Properties p) {
		try {
			...
			// for 문으로 설정 파일 안에 적혀 있는 이름을 기준으로 모두 추가하기
			for(String key : p.stringPropertyNames()) { 
				// xml 파일 안에 들어있는 모든 값을 문자열 형태로 변경해서 검색할 수 있도록 함
				// 모든 키를 담을 String[] 을 생성해서 key 들을 모두 담아줌 
				// 단 , 를 기준으로 
				String values[] = p.getProperty(key).split(",");
				
				// 각각 변수에 담아서 생성
				// 설정값 안에 있는 내용들을 모두 String 에 담아준 후에 가져온 것이기 때문에
				// 정수는 모두 String 에서 -> int 로 변경해서 사용해야함
			...
			// 정렬을 할 때는 정렬하는 기준
			// 정렬할때는 이름이면 이름 년도별이면 년도별 몇부작~ 정렬하는 기준
		    // 담겨있는 드라마들을 다시 정렬하겠다. 정렬기준 Drama[0] = name; 
